<template>
	<view class="message-list">
		<view class="filter-box" @touchmove.stop.prevent :style="showFilterBar ? 'height:100%;' : ''">
			...
			<view class="select-box" v-if="showFilterBar">
				...
				<view class="select-group">
					<view :class="[item.checked ? 'select-item active' : 'select-item']"
						v-for="(item, index) in filterType" :key="index" @click="filterMsgType(item)">{{ item.name }}
					</view>
				</view>
		...
		<view class="list-box">
				...
				<view class="msg-card" @click="viewMsgDetail(item)">
					<view class="msg-content">
						<view class="msg-type">
							...
							<view v-if="item.mbuzType != 401001" class="more-icon"
								@click.stop="msgOperate(item.appNewsUid)">
								<image src="../../../static/images/common_icon/more.png"></image>
							</view>
						</view>
						<!-- <view class="tips" v-if="item.isDefaultHead && !item.isHistory">
							{{ hqkCurrentPerson.name }}，您好，您的信息如下</view> -->
						<view class="tips">{{ item.head }}</view>
			...
			<view class="load-bar" v-if="listData.length > 0">
		...
		methods: {
			...
			getMsgList() {
				...
				self.hqkFuncRequest(self, {
					requestUrl: self.$hqk_api.MESSAGE_NEWLIST,
									...
									if (typeof item.content == 'string') {
										try {
											...
											if (typeof obj == 'object' && obj) {
												item.head = JSON.parse(item.content).head
												for (let i in JSON.parse(item.content).content) {
													let arr = []
													arr = [i, JSON.parse(item.content).content[i]]
													arrList.push(arr)
												}
												item.contentList = arrList
											} else {
												item.head = item.content
											}
										} catch (e) {
											item.head = item.content
										}
			...
			msgOperate(appNewsUid) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							self.hqkFuncRequest(self, {
								requestUrl: self.$hqk_api.MESSAGE_DROPNEWS,
								token: self.$loginStatus.loginStatus.token,
								data: {
									appNewsUid: appNewsUid
								},
								showErrorModal: true,
								success: res => {
									console.log(res)
			...
			viewMsgDetail(item) {
				...
				let self = this
				self.hqkFuncRequest(self, {
					requestUrl: self.$hqk_api.MESSAGE_READNEWS,
					token: self.$loginStatus.loginStatus.token,
					data: {
						appNewsUid: item.appNewsUid
					},
					success: res => {
						console.log(res)
					},
